...
class TagCrudService:
    ...
    async def get_by_id(self, tag_id: str) -> TagResponse:
        ...
        if not tag:
            raise AppError("Tag não encontrada", "tag_not_found", 404, {"tag_id": tag_id})
    ...
    async def update(
        ...
        if not updated:
            raise AppError("Não foi possível atualizar a tag", "tag_update_failed", 500, {"tag_id": tag_id})
    ...
    async def delete(self, tag_id: str) -> None:
        ...
        if not deleted:
            raise AppError("Não foi possível excluir a tag", "tag_delete_failed", 500, {"tag_id": tag_id})
